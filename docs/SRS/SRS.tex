...
\begin{longtable}{| c | p{4cm}| p{6cm}|}
    ...
    \hline
    FR11 & The system should notify the user when it is unable to detect their hands. & This will let the user know if they need to adjust their camera input settings (camera angle, lighting etc.) so the system can accurately access their hand signs. \\
    \hline
...
\hline
UHR4 & The system should tell the user if the their hands are not detectable. & The user should know if they need to change their body posture, camera angle, lighting etc. for the system to accurately give them proper feedback. & The developers shall certify the product complies with these requirements by ensuring the system gives user feedback where appropriate.\\
\bottomrule
\end{tabular}
\end{table}
